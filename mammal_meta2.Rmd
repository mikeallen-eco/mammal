...
mean_reads_df %>%
  ...
  summarise(mean_mean = mean(mean_reads),
            ...
            se = sd_mean/sqrt(n))
```
# cumulative detection probability - flying squirrel, metabarcoding
...
ggsave("figures/species_accumulation_curve_roll_soil4.png",
...
DR_psi._p.method_partA.r$summary[1:17,c(1:3, 7)]
...
predbeta <- matrix(NA, nrow = nsamp, ncol = nspec.r))
for(i in 1:nsamp){
  for(j in 1:nspec.r){
    for(k in 1:nmethods){
...
ll = apply(predp[,,1], 2, function(x) quantile(x, prob = c(0.025))),
ul = apply(predp[,,1], 2, function(x) quantile(x, prob = c(0.975))),
...
ll = apply(predp[,,2], 2, function(x) quantile(x, prob = c(0.025))),
ul = apply(predp[,,2], 2, function(x) quantile(x, prob = c(0.975))),
species = gsub(colnames(m_sp.r[,c(3,5:9,11:12,14:15,18:21)+4]), 
              pattern = "_", replacement = " ")
) %>% mutate(method = "Soil")
...
sp_beta_plot <- data.frame(par = names(pm)[1:16], beta = pm[1:16],
           ...
           psi_q97.5 = 
             apply(psi_mat, 2, function(x) quantile(x, prob = c(0.025, 0.975)))[2,]) %>%
  mutate(species = colnames(m_sp.r)[c(1,3,5:12,14:15,18:21)+4]) %>%
  mutate(species = gsub(species, 
                        pattern = "_", replacement = " ")) %>%
  mutate(species = forcats::fct_reorder(species, p_roller),
         naive_occ = naive_occ,
...
(psi_plot <- sp_beta_plot %>%
  ...
  geom_errorbar(aes(xmin = psi_q2.5, 
                ...
                width = 0, color = "black", size = .75) +
  geom_point(aes(x = psi, y = species), size = 3.5) +
  geom_point(aes(x = naive_occ.r, y = species), size = 3.5, shape = "x", color = "red") +
  theme_bw() +
  theme(text = element_text(size = 15),
        axis.text.y = element_blank()) +
  labs(x = "Occupancy", y = "")
