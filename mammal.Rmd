...
---
# Introduction
This code can be used to reproduce all analysis and figures in the manuscript entitled: "Sampling environmental DNA from trees and soil to detect cryptic arboreal mammals". 

The paper can be found here:
https://doi.org/10.1038/s41598-023-27512-8

# Load libraries and data, and format data for analysis
Some data formatting code adapted from Kery and Royle (2015) Applied Hierarchical Modeling.
```{r}
...
# read in sample and site information
samps <- read_xlsx("data/sample_info.xlsx")
samps2 <- read_xlsx("data/sample_info2.xlsx", 
                    sheet = "Tree  Sample Data") %>%
  mutate(tree = c(paste0("T0", 1:9), paste0("T",10:21)))

# read in data for qPCR vs. metabarcoding comparison
met <- read.csv("data/qPCR_metabarcoding_compare.csv")

# read in and add lineage column to metabarcoding OTU read data
m_raw <- read.csv("data/OTU_table.csv") %>%
  mutate(phylum = "phylum",
         lineage = paste0("r__Chordata;c__", class, 
...
tax <- m_raw %>%
  ...
  select(species, common, phylum, class, order,
         fam = family, genus, sp)

# make a version of OTU data with taxonomy info
m_otu_tax <- m_raw %>%
  select(-OTU_num, -psim, -sequence) %>%
  select(common, starts_with("Sample"), species, phylum, 
         class, order, fam = family, genus, sp, lineage)

# make a simplified version of the OTU data with just common name
m_otu <- m_otu_tax %>%
  select(common, starts_with("Sample"))

# make version of OTU data with reads summarized by species
  # multiple steps
m_sp_temp <- m_otu %>%
  group_by(common) %>%
  summarize_all(sum) %>%
...
m_flip <- m_flip[-1,] # remove top row with species names
...
obs.occ # view number of sites each species are seen at
...
# Format occupancy data using subset of roller samples to match soil
  # i.e., the analyses presented in Supplementary Material
...
for(s in 1:n_methods){
  for(j in 1:n_samples){
    tree_num <- as.numeric(as.factor(met$tree))[j]
    samp_num <- met$samp_num[j]
    met2 <- met[,c("met_det","q_det")]
...
method = method_mat 
# add placeholder value for NAs so it runs; doesn't affect results
...
# Bundle and summarize metabarcoding vs. qPCR data set for JAGS
...
# save the figure
# ggsave("figures/Fig1B_mean_reads_by_species_method_mammals2.png", 
#        height = 5,
#        width = 7, dpi = 400)
...
obj$data$tax_occ <- calc_n_samples(obj, "tax_abund1", 
                                   groups = Method, 
                                   cols = Samples)

# calculate the number of methods that detected each taxon
obj$data$tax_occ$num_methods <- 
  (obj$data$tax_occ$S>0) +
...
# ggsave("figures/Roller_heattree_abund2.jpg", 
#        width = 7, height = 7, dpi = 400)
...
accum.roll1 <- accumcomp(roll_mat_1samp, 
                     y=data.frame(method = 
                                    rep("roll1",
                                        nrow(roll_mat_1samp))),
                     factor='method', 
  method='exact', conditioned=FALSE, 
  plotit=FALSE)
accum.roll1
...
Community occupancy model on only mammals. Code adapted from Kery and Royle (2015) Applied Hierarchical Modeling, Vol. 1 book, section 11.7.2.
```{r}
# update n species so that it reflects mammals only
nspec <- dim(y_mam)[3]

# Augment data set: prior on Ntotal
nz <- 40 - nspec         
...
naive_occ <- apply(apply(m_sp_detnon[, , c(1,3,5:12,14:15,18:21)], 
                         c(1, 3), max, na.rm = T), 2, sum)/21
...
Note: currently, to run this, you need to first run the previous chunk to create p_roller_mat and other objects. Also, this plot is saved here as an object and then used to make a 2-panel plot in a chunk further down in this document.
```{r}
# create matrix for roller posterior samples
  # note: nrow = 6000 because there are 6000 draws from the posterior
...
# Note: this plot object will be combined with another below
  # to make the final Fig. 5
```
...
sites <- samps2 %>%
  select(tree, gps = 2, loc = 3)
  
# join the OTU data with information about the sites
s <- m_sp %>%
  left_join(sites)
...
To compare qPCR vs. metabarcoding detection in Big Brown Bat.
...
To compare qPCR vs. metabarcoding detection of Big Brown Bat.
Run time was approximately 30 seconds with parallel = T.
```{r}
# Initial values
y_no_na <- y_multi
...
```
# Create Fig5 (both panels) - cumulative detection, meta vs qPCR
Graph cumulative probability of detection in occupied trees with increasing survey effort. Note: need to load plot for flying squirrel first using code from further up in document.
```{r}
...
# ggsave("big_brown_bat_cumulative_detection.pdf", 
#        width = 7, height = 6, dpi = 600, device=cairo_pdf)

# version with Flying Squirrel and Brown Bat side-by-side
# note: need to create the flying squirrel plot using code
    # from a code chunk earlier in this document
# dev.new(width = 10, height = 4)
the_plot <- gridExtra::grid.arrange(flying_squirrel, 
                                    brown_bat, 
                                    ncol = 2)
ggsave("figures/Fig5_squirrel_bat_cummulative_p4.png", 
...
test_soil <- apply(m_sp_array_soil, c(1,2), sum); test_soil
test_roll <- apply(m_sp_array_roll, c(1,2), sum); test_roll
```
# Create Fig. S1. Non-metric multidimensional scaling (NMDS) plot
Code modified from:
...
# format site and method covariate data
...
Community occupancy model on only mammals, omitting any roller samples that do not have a corresponding soil sample. This model forms the basis for Figure S3 in the Supplemental Material. Code adapted from Kery and Royle, AMH 1 book, section 11.7.2.
```{r}
# update n species
...
DR_psi._p.method_partB.r <- 
  jagsUI::jags(win.data.r, inits, params2, 
               "scripts/DR_psi._p.method.txt", 
               n.chains = nc, n.thin = nt, n.iter = ni, 
               n.burnin = nb, parallel = TRUE)
...
```
# Create Fig. S3 - Plot output from community occupancy model (reduced data set)
The output from the community occupancy model evaluating Roller and Soil detection probability (version run on the reduced data set reported on in the Supplemental Material).
```{r}
# load the models
DR_psi._p.method_partA.r <-
...
dev.new(width = 7, height = 5)
the_plot <- gridExtra::grid.arrange(beta_plot, p_plot, ncol = 2)
# ggsave("figures/FigS3_occ_plot5.r.png", plot = the_plot, width = 7, height = 5, dpi = 400)

# get mean, 2.5% and 97.5% quantiles for beta of sampling method
...
# [1] 2.165275
```
# Create Fig. S4 - cumulative detection probability (squirrel), reduced data set
Comparing cumulative detection probability for flying squirrel with metabarcoding of soil and roller samples (reduced data set reported on in Supplemental Material). Note: you'll need to run the previous chunk to get some of the data required for this plot.
```{r}
DR_psi._p.method_partA.r <-
  readRDS("output/DR_psi._p.method_partA.r.rds")
...
# ggsave("figures/FigS4_flysquirrel_cumulative_p.r.png", 
#        width = 4, height = 4,
#        dpi = 400)

# ~ 3 and ~ 11 samples required, respectively
```
# Create Fig. S2 - Species accumulation curves (reduced data set)
This is the species accumulation curve analysis run on the reduced data set as reported on in the Supplemental Material. Some code adapted from here: https://rpubs.com/Roeland-KINDT/694021
and here: http://biological-complexity.blogspot.com/2018/01/on-species-accumulation-curves.html
```{r}
# Make empty martices for the spp accumulation curves
...
# save the plot
# ggsave("figures/FigS2_species_accumulation_curve_roll_soil4.r.png",
#        dpi = 600, width = 6, height = 4)
```