...
# for later
#  mutate(lineage = paste0("r__Chordata;c__", class, ";o__", order, ";f__", fam))
...
m_sp <- cbind.data.frame(m_sp_temp2[,1:4], sp_mat) %>%
...
sp_mat_temp <- matrix(as.numeric(as.matrix(m_sp_temp2[,5:26])), 
                   nrow = 126, ncol = 22,
                    byrow = F) %>%
  as.data.frame()
colnames(sp_mat_temp) <- colnames(m_sp_temp2[,5:26])
...
str( win.data <- list(ysum = ysum, M = nrow(ysum), J = J, nspec = dim(ysum)[2], R = matrix(c(5,0,0,1), ncol = 2), df = 3) )

# Specify model in BUGS language
sink("scripts/psi._p.txt")
cat("
model {
...
psi._p. <- jagsUI::jags(win.data, inits, params, "scripts/psi._p.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
par(mfrow = c(3,3))   ;   jagsUI::traceplot(psi._p., c('mu.eta', 'probs', 'Sigma', 'rho'))
print(psi._p., 3)


detprob <- as.data.frame(psi._p.$summary[27:48,]) %>%
  rename(q2.5 = 3, q97.5 = 7, q25 = 4, q75 = 6) %>%
  mutate(species = names(obs.occ),
         obs.occ = obs.occ) %>%
  mutate(species = forcats::fct_reorder(species, mean, .desc = F))

occprob <- as.data.frame(psi._p.$summary[5:26,]) %>%
  rename(q2.5 = 3, q97.5 = 7, q25 = 4, q75 = 6) %>%
  mutate(species = names(obs.occ),
         obs.occ = obs.occ,
         naive.occ = obs.occ/n_trees) %>%
  mutate(species = forcats::fct_reorder(species, mean, .desc = F))
...
ggsave("figures/detprob_psi._p.png", height = 6, width = 7, dpi = 400)
...
ggsave("figures/occprob_psi._p.png", height = 6, width = 7, dpi = 400)
...
m_sp_mean_no_blanks <- m_sp_mean[rowSums(m_sp_mean[,2:23])>0,] %>%
  mutate(tree = paste0("T", substr(site, 2,3)),
         method = substr(m_sp_mean_no_blanks$site,1,1)) %>%
...
env.data <- m_sp_mean_no_blanks[,24:26]
m_sp_mean_no_blanks_mat <- as.matrix(m_sp_mean_no_blanks[,2:23])
...
(nmds_plot <- ggplot(sites.long1) +
  ...
  coord_fixed(ratio = 1)
ggsave("figures/reads_ordination_by_method_site.png", height = 6, width = 6, dpi = 400))
...
#         "output/DR_psi._p.method_partA.rds")
DR_psi._p.method_partA <-
  readRDS("output/DR_psi._p.method_partA.rds")
...
#         "output/DR_psi._p.method_partB.rds")
DR_psi._p.method_partB <-
  readRDS("output/DR_psi._p.method_partB.rds")
...
p_roller_mat <- matrix(NA, ncol = 15, nrow = 1500)
p_soil_mat <- matrix(NA, ncol = 15, nrow = 1500)
psi_mat <- matrix(NA, ncol = 15, nrow = 1500)
for(i in 1:15){
  betalp1 <- all10[,i]
  lp <- all10[,i+41]
  lpsi <- all10[,i+81]
...
naive_occ <- apply(apply(m_sp_detnon[, , c(1, 3:10, 12:13, 16, 18:20)], c(1, 3), max, na.rm = T), 2, sum)/21
sp_beta_plot <- data.frame(par = names(pm)[1:15], beta = pm[1:15],
           beta_q2.5 = cri[1,][1:15],
           beta_q97.5 = cri[2,][1:15],
  ...
  mutate(species = colnames(m_sp)[c(5,7:14,16:17,20,22:24)]) %>%
...
ggsave("figures/occ_plot4.png", plot = the_plot, width = 11, height = 5, dpi = 400)
...
mean_reads_df <- data.frame(
  ...
  mutate(species = forcats::fct_reorder(species, mean_reads))
...
mean_reads_df %>%
  ...
  theme_bw() +
  theme(text = element_text(size = 14)) +
  labs(y = "", x = "Mean number of reads (log+1)") +
  scale_x_log10()
ggsave("figures/mean_reads_by_species_method.png", height = 5,
       width = 7, dpi = 400)
```
